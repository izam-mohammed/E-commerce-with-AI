...
def view_prod(request, pid):
    ...
    prdt = PrdVariation.objects.filter(
        prd_id_id=prd1.prd_id_id
    )  # display other product variations
    if request.user.is_authenticated:
...
def inc_qty(request, item_id):
    ...
    if prd.stock > 0:
        ...
        context = {
            ...
            "price": cart_item.prd_var.cur_price,
            "tot": tot,
            "Success": True,
        }
...
def create_order(request, ad_id):
    ...
    if "coupon" in request.session:
        ...
        new_order = Order.objects.create(
            ...
            tot_amount=tot,
            del_add_id=ad_id,
            coupon_apply_id=coupon_id,
            status="Pending",
        )
        del request.session["coupon"]
    else:
        new_order = Order.objects.create(
            user=request.user, tot_amount=tot, del_add_id=ad_id, status="Pending"
        )
    item_string = " ".join(item_list)
    notification_description = (
        "Your order containing " + item_string + " has been placed."
    )
...
def r_paid(request, or_id):
    ...
    client = razorpay.Client(
        auth=("rzp_test_DLTeq2nNMzXQkj", "bmng64HRS0muDq1yCGX0m8YS")
    )
    payment = client.order.create(
        {"amount": r_tot, "currency": "INR", "payment_capture": "1"}
    )
